...
def gen_set( data, group, prefix, suffix, inds, t, n ):
    ...
    for i in inds:
        ...
        for s in gsuffs:
            ...
            if key in data:
                ret.update( data[key] )
                for ngram in data[key]:
                    ngram_ind_map[ ngram ].append( i )
    ngram_ind_map.pop('', 0)   #remove the empty index
    ...
    for ngram in all_set:
        ...
        for i in inds:
            ...
            if i in map1[ngram] and i in map2[ngram] and i in map3[ngram]:
                counts[ngram] += 1
    counts.pop("", 0)   #remove the empty count
...
#get file and preprocess data to binary form (subject either contains ngram or doesn't contain it)
df = pd.read_csv(file)
ngrams = pd.DataFrame(df.iloc[:,0].to_numpy().reshape( df.shape[0], 1 ))

#error check that N is consistent with ngram lengths
if int(N) != len(ngrams.iloc[0,0]):
    print(f"ERROR: N ({N}) DOES NOT MATCH NGRAM LENGTH ({len(ngrams.iloc[0,0])}). EXITING")
    exit()

#preprocess data csv into form semi-consistent with szo's venn generator (minus the n specifier)
df = df.drop( df.columns[0], axis=1 )
df1 = df.applymap( lambda x: 1 if x > 0 else 0 )
data = pd.DataFrame( df1.values * ngrams.values, columns=df.columns )
